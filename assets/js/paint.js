...
function configureListeners() {
  ...
  for (let i = 0; i < images.length; i++) {
    // Set iteration to apply below event listeners to each color block
    document
      .getElementById(images[i].id)
      .addEventListener("mouseover", addOpacity, false); // Activates event listener to dim image when mouseover occurs
    document
      .getElementById(images[i].id)
      .addEventListener("mouseout", removeOpacity, false); // Activates event listener to undim image when mouseout occurs
  }
}
...
function addOpacity(event) {
  if (!this.classList.contains("dim")) {
    // Set conditional to determine if corresponding ("this") color block (img tag) does NOT have "dim" CSS property (classList)
...
function removeOpacity(event) {
  // removeOpacity triggers when mouseout event occurs
  if (this.classList.contains("dim")) {
    // Set conditional to determine if the "dim" CSS property (classList) of the color block (img tag) is currently active
